...
<InputWrapper title={title} horizontal={horizontal} helpText={helpText}>
  <FieldWrapper expanded={expanded} loading={loading} prefix={prefix} suffix={suffix}>
    <div class="select control {loading ? ' is-loading' : ''} {leftAligned ? ' is-pulled-left' : ''}">
      <select on:input={event => onInput(type.fromString(event.currentTarget.value))}>
        {#each valueStrings as value}
          <option value={value} selected={value === type.toString(selected)}>
            {value}
          </option>
        {/each}
      </select>
