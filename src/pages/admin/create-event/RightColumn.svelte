...
<Column>
  ...
  <CheckboxInput
    ...
    onChange={defaultNotAttend => updateEvent({ ...event, defaultAttend: !defaultNotAttend })}
  />
  <CheckboxInput
  ...
  <SelectInput
    ...
    onInput={period => updateRepeat(period && repeat ? ({ ...repeat, period }) : null)}
    title="Repeat"
  />
  <TextInput
    type={dateType}
    value={repeat?.repeatUntil}
    onInput={repeatUntil => updateRepeat(repeatUntil && repeat ? { ...repeat, repeatUntil } : null)}
    title="Repeat Until"
